...
| Feature | Status | Notes |
|---|---|---|
| Test without connecting | ✅ | A delay test with no connection open now brings the core up for itself and shuts it down 30 seconds after the last test. That core is not a connection: TUN is off, so nothing touches the machine's routing, and nothing is announced to the UI as connected. `connect()` takes the core over from it, and a group test firing dozens of concurrent requests starts exactly one, through a startup gate. |
| Ping that measures what the user asked | 📋 | Two problems with one cause. Ours is the full path: sing-box's Clash `/delay` times the TCP connect, the TLS or REALITY handshake, the proxy's dial to the target and the request. NekoBox reports `speedtest.UrlTest` in RTT mode — the request sent twice over one keep-alive connection, timed from headers-written to first-byte, so every handshake is excluded. Side by side on the same nodes: 292 ms against 79, 158 against 68, 207 against 96. The worse problem is the verdict, not the number: `/delay` reports nodes as dead that carry traffic perfectly. One hysteria2 node failed it at 5, 10 and 20 second timeouts while answering through the tunnel in 53 ms, five times out of five — and its three siblings, byte-identical but for the IP, passed. A test whose whole purpose is "can I use this server" must answer by using it, which is what the NekoBox method does and `/delay` only approximates. Switching means measuring through the mixed inbound ourselves, and a request can only be aimed at one outbound at a time, so tests serialise where they are concurrent today — which makes the progress row below a prerequisite rather than a nicety. **Discuss first.** |
| Progress for a group test | 📋 | Testing a group of hundreds gives one status line and no sense of how far along it is or what it found. Needs a progress indicator and a way to stop it. Worth pairing with a cap on how many tests run at once: a group test currently fires one request per profile simultaneously. |

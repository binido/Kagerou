...
| Per-profile delay test | 🟡 | Two columns, one measurement. Both "TCP ping" and "URL test" issue the same `/proxies/{name}/delay` request through the Clash API — a URL test through the proxy — and differ only in how the result is printed. There is no TCP ping. `getReachabilityAwarePing` then invents a relationship between them, showing the ping value only when the URL result is good and substituting "Unavailable" otherwise, so the UI implies one test corroborates the other when both are the same number. Sorting by ping reads that invented value. See the row below. **Discuss first.** |
| Delay testing that means something | 📋 | The two tests should answer different questions, as they do in NekoBox. A TCP ping opens a socket to the profile's own `server:port` and times the handshake: it says the server is alive and how far away it is, proves nothing about the proxy, and needs no running core — so it works while disconnected and is cheap enough to sweep hundreds of profiles. A URL test measures the whole path through the proxy to a target URL, which is the number that actually matters, but needs the core running and is slow one profile at a time. The open question is whether to keep both: dropping TCP costs the ability to test anything before connecting and to triage a large subscription quickly, unless the core is made to run for tests independently of the connection. Note also that a TCP ping reaches the proxy server directly from the user's own address, which the URL test does not. Whatever is decided, `getReachabilityAwarePing` goes: each column shows its own result or nothing. **Discuss first.** |

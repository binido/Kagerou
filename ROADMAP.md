...
| Theme-aware icon | 💡 | Only partly possible, so worth deciding what it is actually for before starting. `WindowEvent::ThemeChanged` reaches the frontend, and `Window::set_icon` can swap the taskbar icon at runtime on Windows and Linux — but it is a no-op on macOS, where windows have no icons at all. The bundle icon is baked into the `.app` and `.exe` and cannot change at runtime; rewriting it on macOS would break the code signature. What can genuinely follow the system theme is a tray icon, via `set_icon` and macOS's template mode — which makes this an extension of [System tray](#platform-integration) rather than a standalone feature. |

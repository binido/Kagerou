...
| Provider headers | 📋 | Providers send the traffic used and the expiry date (`subscription-userinfo`), an announcement (`announce`) and a support link (`support-url`) with the subscription; Happ shows them. Only `profile-title` is read today. |

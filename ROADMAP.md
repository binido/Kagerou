...
| Per-profile delay test | ✅ | Two measurements that answer different questions. The ping opens a TCP socket to the profile's own `server:port` and times the handshake: it needs no running core, so it works while disconnected, and says the server is up without claiming the proxy works. The URL test measures the whole path through the proxy via the Clash API and reports that latency, which is the number the connection actually feels like. Hysteria2 and TUIC ride QUIC, so a TCP probe would time out on a healthy server — their ping reads `n/a` rather than lying. Sorting by ping orders on the URL latency and falls back to the reachability ping. Dropping the ping in favour of the URL test alone was considered and rejected: it is the only test that works before connecting, and the only one cheap enough to sweep a subscription of hundreds. Worth remembering that it reaches the proxy server directly from the user's own address, which the URL test does not. |

...
| System proxy | ✅ | Confirmed on macOS: the proxy is set on connect and cleared on disconnect. sing-box's own `set_system_proxy` on the mixed inbound points the OS proxy at `127.0.0.1` and the proxy port on start and restores it on a clean stop. A stop sends SIGTERM before SIGKILL so that cleanup runs; Windows has no such signal, so there Kagerou resets the proxy through WinINet after the core exits and at startup, but only while it still points at that port. Exclusive with TUN mode, and applies on the next connection. Windows and Linux have not been run; the WinINet path has only been type-checked against the Windows target. Still open: a crash on macOS or Linux leaves the proxy pointing at a dead port until the next connect and stop, and Linux desktops other than the ones sing-box supports will not see it at all. |
| Inbound listen address and port | 🟡 | Both ports are settings (default 2080 for the mixed inbound, 9090 for the Clash API), locked while connected and kept off the test core's 2081 and 9091. Both still listen on `127.0.0.1` only. Missing: an "allow LAN access" toggle that binds the mixed inbound to `0.0.0.0`. The inbound has no users, so that toggle opens the proxy to anyone on the network - it wants a username and password with it. The Clash API stays on loopback either way, it has no secret. **Discuss first.** |
...
| Subscription formats | ✅ | Plain or base64 URI lists, Clash-style YAML (`proxies:`), sing-box JSON (`outbounds`), Xray JSON (one config, or the array of configs panels serve to Xray clients), Shadowsocks SIP008 (`servers`; a server that needs a plugin is left out). Subscriptions are fetched with a `singbox` User-Agent, which is what makes panels such as Remnawave answer with sing-box JSON rather than Xray. From Xray JSON each config becomes one profile named by its `remarks`; a config that routes through a balancer ("Auto \| Best server", whitelist-bypass pairs) or reaches its server through another proxy is left out, because one profile cannot hold an automatic pick or a chain. |
| Xray core for `xhttp` | 📋 | sing-box has no `xhttp` transport, so servers that need it - typically the whitelist-bypass ones - are left out of every import. Carrying them means a second core: another binary run with elevated privileges, a second config generator, a second pinned download. **Discuss first.** |
| Provider routing from Xray configs | 📋 | Xray configs carry the provider's own rules (domains sent direct, QUIC blocked). They are ignored: traffic follows the routing page. Importing them decides where traffic goes and depends on what a preset is. **Discuss first.** |
...
| Feature | Status | Notes |
|---|---|---|
| Dashboard, groups, routing, connections, logs, settings | ✅ | Six pages, all driven by the real backend. Subscriptions used to have a page of their own and now live on the groups page, with `/sources` redirecting there. |
...
| Localisation | ✅ | English and Russian. The one hole is closed: the sources table holds the refresh time as unix milliseconds instead of the English sentence the backend used to write, and the groups page formats it with `Intl.RelativeTimeFormat`, so it both translates and advances. Migration 0011 clears the old prose — it cannot be turned back into a time, so those subscriptions show no refresh time until their next refresh. The second hole went the same way: a profile's test result was stored as the English sentence the interface showed, so translating it meant matching on that text and sorting by latency meant parsing a number out of it. Migration 0012 turns it into a kind and a number, and the colour is derived from them. That also fixed "remove unavailable": it filtered on the display colour, which a latency over 400ms carries too, so a working-but-slow server was deleted along with the ones that never answered. The last hole was the errors themselves: every command answered with a bare English sentence which the window then showed as-is, so a Russian user read "database error: no such table". Commands answer with a code now, the window translates it, and the backend's own wording goes to the console rather than in front of anyone. |
...
| Log viewer | ✅ | Streams the core's output live, with level detection. The three defects the audit found are fixed: the column shows `HH:MM:SS` while the entry keeps its ISO timestamp, the INFO level uses `text-body` like the two levels below it, and rows carry `content-visibility: auto`, so off-screen ones skip layout and paint but stay in the DOM for find-in-page and selection. The audit's snippet put that property on the list container, which skips nothing — the container is the scrolled content and is never fully off screen; it belongs on the row, which is also what its intrinsic height describes. The same pass fixed a crash the audit had understated: the character class escaping the search query was malformed, so nothing was escaped and a query containing `(` or `[` threw inside render — with no error boundary above it, that empties the window. The escaping now lives in `app/src/lib/log-search.ts` with tests. A later pass found the stream itself stopped after 500 lines: the forwarder tracked its position by the length of the supervisor's ring buffer, which stops growing at the cap, so every later line looked already sent. `Supervisor::logs_since` counts what the process produced instead. |
| Connection list | ✅ | A connections page lists what is open through the core, newest first: destination, network, the profile it leaves through (or direct / blocked), the rule that sent it there, bytes both ways and how long it has been open. Each row closes on its own, and "close all" drops the lot. It polls `/connections` once a second, only while the page is open, and a failed read keeps the last list rather than raising a toast per tick. The exit is found by looking for a profile anywhere in the chain, since the chain also holds the `proxy` selector. |
...
| Release builds | 🟡 | `.github/workflows/release.yml` builds macOS (both architectures), Linux and Windows bundles from a `v*` tag and attaches them to a draft release, after checking the tag against `tauri.conf.json`. Proven on `v0.1.0`: dmg, deb, rpm, AppImage, msi and exe all built. The Windows job also zips `Kagerou.exe` with `sing-box.exe` into a portable build; settings still go to `%APPDATA%`, and nothing installs WebView2 for it. Nothing is downloadable until that draft is published. Linux ships x86_64 only. |
...
| Update notification | ✅ | On launch the app asks GitHub for the latest release and, if it is newer than the running build, the sidebar offers it: an installable build downloads it in place (see In-app updates), the portable and dev builds link straight to the release page. Silent when the check fails or there are no releases. The notice and the repository link in the settings footer both hand the URL to the `opener` plugin, whose permission is scoped to this repository and its subpaths — the release URL arrives from the GitHub API, so an unscoped opener would let that answer choose where the browser goes. Confirmed in a built bundle on macOS: both links open the system browser. Windows and Linux go through the same plugin but have not been run. |
...
| Linux repositories | 📋 | Today a Linux user downloads a loose `.deb`, `.rpm` or AppImage from the release page, and only the app itself updates it afterwards, outside the system package manager. Repositories fix that: an apt repository (Debian/Ubuntu, hostable from GitHub Pages), COPR (Fedora), OBS (openSUSE, and it can build for several distros at once). Worth doing in that order — each is independent, and each is a chunk of packaging work rather than app work. |
...
| In-app updates | 🟡 | The sidebar notice downloads the release in place with a progress bar, then offers a restart; `tauri-plugin-updater` checks the download against the public key in `tauri.conf.json` before anything is replaced. The VPN stays up during the download and is disconnected, along with the test core, only when the user asks for the restart: the Windows installer ends the process past the exit hook that stops sing-box and cannot overwrite a running `sing-box.exe`. Covers the macOS app, the NSIS and MSI installers, AppImage, `.deb` and `.rpm` (the last two install through `pkexec`). The portable Windows build and dev builds carry no bundle type, so they keep the link to the release page. Release builds sign the artifacts with the `TAURI_SIGNING_PRIVATE_KEY` secret, and a last job fails the release if `latest.json` lacks an installer. Proven end to end on macOS: an installed 0.7.0 downloaded 0.7.1 from the sidebar, restarted and came back as 0.7.1. Windows and Linux have not been updated in place yet. The signature is not bound to the version (`requireSignedVersion` stays off, since Tauri CLI 2.11 does not record it), so a tampered `latest.json` could pair a new version number with an older signed build. |
...
- [x] **Relative timestamps never advance, and English leaks through the parser.**

...
| Ping that matches other clients | 📋 | Our number is the full path: sing-box's Clash `/delay` times the TCP connect to the proxy, the TLS or REALITY handshake, the proxy's own dial to the target, and the request. NekoBox reports something else — `speedtest.UrlTest` in RTT mode, which sends the request twice over one keep-alive connection and measures the second from headers-written to first-byte, so every handshake is excluded. Measured side by side on the same nodes at the same moment: 292 ms against 79, 158 against 68, 207 against 96 — two to four times, which is the whole of the difference users notice. Neither is wrong, but ours is not comparable to any other client, and the smaller number is the better signal for choosing between nodes. Switching means measuring it ourselves through the mixed inbound rather than asking the Clash API, and that has a cost: a request can only be aimed at one outbound at a time, so tests serialise where they are concurrent today. **Discuss first.** |

...
| Update notification | ✅ | On launch the app asks GitHub for the latest release and, if it is newer than the running build, the sidebar links straight to it. Silent when the check fails or there are no releases. The notice and the repository link in the settings footer both hand the URL to the `opener` plugin, whose permission is scoped to this repository and its subpaths — the release URL arrives from the GitHub API, so an unscoped opener would let that answer choose where the browser goes. Confirmed in a built bundle on macOS: both links open the system browser. Windows and Linux go through the same plugin but have not been run. |

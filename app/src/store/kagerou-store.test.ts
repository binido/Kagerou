...
describe('hydrate', () => {
  ...
  it('still reports the failure when the data directory cannot be read either', async () => {
    ...
    expect(useKagerouStore.getState().hydrateError).toEqual({
      dataDir: '',
      message: en.errors.storage,
    })
  })
...
describe('tray intents', () => {
  it('the tray connect item goes through the same toggle the button does', async () => {
    let fire: () => void = () => {}
    api.onTrayToggleConnection.mockImplementation((h: () => void) => {
      fire = h
      return Promise.resolve(() => {})
    })
  ...
  it('the tray profile item goes through the same select the list does', async () => {
    ...
    api.onTraySelectProfile.mockImplementation((h: (id: string) => void) => {
      fire = h
      return Promise.resolve(() => {})
    })
...
describe('backend event handling', () => {
  ...
  it('a traffic sample event replaces the latest speed sample', async () => {
    ...
    api.onTraffic.mockImplementation((h: (e: TrafficEvent) => void) => {
      handler = h
      return Promise.resolve(() => {})
    })
  ...
  it('traffic history keeps the last minute of samples, oldest first', async () => {
    ...
    expect(history.at(-1)).toEqual({
      download: (TRAFFIC_HISTORY_LIMIT + 9) * 2,
      upload: TRAFFIC_HISTORY_LIMIT + 9,
    })
  })
  ...
  it('connecting asks where the tunnel comes out', async () => {
    ...
    api.lookupExitLocation.mockResolvedValue({
      ip: '81.2.69.142',
      city: 'London',
  ...
  it('a traffic sample event replaces sessionTraffic with the backend-reported totals', async () => {
    ...
    handler({
      ...
      up: 1,
      down: 2,
      uploadTotal: 250_000_000,
      downloadTotal: 1_900_000_000,
      activeConnections: null,
    })
    handler({
      kind: 'sample',
      up: 3,
      down: 4,
      uploadTotal: 260_000_000,
      downloadTotal: 1_950_000_000,
      activeConnections: null,
    })

    expect(useKagerouStore.getState().sessionTraffic).toEqual({
      download: 1_950_000_000,
      upload: 260_000_000,
    })
  })
  ...
  it('a log event is appended and level-detected from the message text', async () => {
    ...
    api.onLog.mockImplementation((h: (line: string) => void) => {
      handler = h
      return Promise.resolve(() => {})
    })
...
describe('backend failure reporting', () => {
  ...
  it('rolls the group panel back from the snapshot when the open state fails to persist', async () => {
    const stored: ProfileGroup[] = [
      { id: 'g1', label: 'G1', kind: 'custom', profileIds: [], open: false },
    ]
    ...
    const stored = [
      {
        id: 'bypass-lan',
        label: 'Bypass LAN',
        description: 'Private ranges go direct',
        enabled: false,
      },
    ]
  ...
  it('reverts the rule selection from the snapshot when the backend refuses it', async () => {
    ...
    expect(useKagerouStore.getState().routingRules.find((rule) => rule.id === 'r2')?.selected).toBe(
      true,
    )

    await vi.waitFor(() =>
      expect(
        useKagerouStore.getState().routingRules.find((rule) => rule.id === 'r1')?.selected,
      ).toBe(true),
    )
    expect(toast.error).toHaveBeenCalledWith(en.errors.storage)
  })
  ...
  it('rolls the rule and the pending-changes flag back when the update fails to persist', async () => {
    const stored: RoutingRule[] = [
      { id: 'r1', match: 'a.com', outbound: 'Direct', selected: false },
    ]
    useKagerouStore.setState({
      routingRules: stored.map((rule) => ({ ...rule })),
      connected: true,
  ...
  it('reverts the theme and the persisted id when the backend refuses the change', async () => {
    ...
    api.getAppState.mockResolvedValue({
      ...emptySnapshot,
      settings: { ...emptySnapshot.settings, theme: 'catppuccin-mocha' },
    })
    ...
    await vi.waitFor(() =>
      expect(useKagerouStore.getState().settings.theme).toBe('catppuccin-mocha'),
    )
  ...
  it('rolls the settings back from the snapshot when persistence fails', async () => {
    useKagerouStore.setState({
      settings: { ...initialState.settings, testUrl: 'http://changed.example/204' },
    })
    ...
    await vi.waitFor(() =>
      expect(useKagerouStore.getState().settings.testUrl).toBe(

...
export function ProfileGroupCard({
  ...
  const profileCount = tc(profiles.length === 1 ? 'counts.vpnOne' : 'counts.vpnMany', {
    count: profiles.length,
  })
  const lastRefresh = source
    ? formatRelativeTime(source.lastRefresh, i18n.resolvedLanguage ?? 'en')
    : ''
    ...
    <Card className="overflow-visible rounded-[10px] border border-hairline bg-surface p-0 shadow-none">
      ...
      {group.open ? (
        <div aria-hidden={!group.open} id={`${group.id}-panel`} role="region">
          ...
          />
          {isSubscription ? (
            <div className="flex items-center gap-1.5 border-t border-hairline px-5 py-3.5 text-[11px] text-muted-copy">
              <Radar aria-hidden="true" className="size-3.5 text-lavender" />
              {t('group.subscriptionNote')}
            </div>
          ) : isDefault ? (
            <div className="flex items-center gap-1.5 border-t border-hairline px-5 py-3.5 text-[11px] text-muted-copy">
              <Folder aria-hidden="true" className="size-3.5 text-lavender" />
              {t('group.defaultNote')}
            </div>
          ) : null}
        </div>

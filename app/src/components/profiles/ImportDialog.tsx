      ...
      <DialogContent className="border-hairline bg-raised text-primary sm:max-w-[520px]">
        <DialogHeader>
          ...
          <DialogDescription className="text-[12px] leading-5 text-muted-copy">
            {clipboardWasEmpty
              ? t('dialogs.import.emptyClipboard')
              : t('dialogs.import.description')}
          </DialogDescription>
        </DialogHeader>
        <form className="space-y-5" onSubmit={handleSubmit}>
          <Field>
            <FieldLabel className="text-[12px] text-primary" htmlFor="import-text">
              {t('dialogs.import.label')}
            </FieldLabel>
            <Textarea
              aria-describedby={draft?.error ? 'import-error' : undefined}
              aria-invalid={Boolean(draft?.error)}
              autoFocus
              className="min-h-[120px] resize-y border-hairline bg-surface font-mono text-[11px]"
              disabled={submitting}
              id="import-text"
            ...
            />
            {draft?.error ? (
              <FieldError className="text-[11px]" id="import-error">
                {draft.error}
              </FieldError>
            ) : null}
          </Field>

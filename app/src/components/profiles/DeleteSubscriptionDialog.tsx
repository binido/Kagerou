      ...
      <AlertDialogContent className="border-hairline bg-raised text-primary sm:max-w-[440px]">
        <AlertDialogHeader>
          ...
          <AlertDialogTitle className="type-display mt-2 text-[23px] text-primary">
            {t('dialogs.deleteSubscription.title', { group: group?.label ?? '' })}
          </AlertDialogTitle>
          <AlertDialogDescription className="text-[13px] leading-5 text-body">
            {t('dialogs.deleteSubscription.description', { count: group?.profileIds.length ?? 0 })}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          ...
          <AlertDialogAction className="bg-bad text-ink hover:bg-bad/85" onClick={onConfirm}>
            {t('dialogs.deleteSubscription.submit')}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>

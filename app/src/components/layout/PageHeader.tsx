    ...
    <header className="flex items-start justify-between gap-6 max-[640px]:flex-col">
      <div className="min-w-0">
        ...
        <h1
          className={
            eyebrow
              ? 'type-display mt-3 text-[34px] leading-none text-primary'
              : 'type-display text-[34px] leading-none text-primary'
          }
        >
          {title}
        </h1>
        {description ? (
          <p className="type-body mt-3 max-w-[610px] text-muted-copy">{description}</p>
        ) : null}
      </div>
      {(actions || status) && (

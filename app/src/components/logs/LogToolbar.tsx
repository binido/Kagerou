  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      <label className="log-toolbar-control log-toolbar-search flex min-w-0 flex-1 items-center gap-3 rounded-lg bg-surface ring-1 ring-inset ring-hairline transition-colors focus-within:ring-2 focus-within:ring-lavender/70">
        <Search
          aria-hidden="true"
          className="size-[17px] shrink-0 text-muted-copy"
          strokeWidth={1.7}
        />
      ...
      <Button
        className="log-toolbar-control shrink-0 gap-2 bg-surface text-[12px] text-body ring-1 ring-inset ring-hairline hover:bg-row-hover hover:text-primary"
        onClick={onClear}
        type="button"
        variant="ghost"
      >
      ...
      <span
        aria-live="polite"
        className={cn('ml-auto shrink-0 text-[12px] text-muted-copy', query && 'text-lavender-hi')}
        role="status"
      >
        {countLabel}
      </span>

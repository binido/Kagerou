...
export function LogViewer({ entries, query }: { entries: LogEntry[]; query: string }) {
  ...
  return (
    <section aria-label={t('viewer.ariaLabel')} className="mt-4 flex min-h-0 flex-1 flex-col">
      <ScrollArea
        aria-label={t('viewer.scrollableAriaLabel')}
        className="min-h-0 flex-1 overflow-hidden rounded-[10px] bg-viewport ring-1 ring-inset ring-hairline"
        id="log-viewport"
        tabIndex={0}
      >
        <div className="min-w-0 font-mono">
          {entries.length ? (
            <div className="space-y-0 px-4 py-4" id="log-list">
              {entries.map((entry) => (
                <LogRow entry={entry} key={entry.id} query={query} />
              ))}
            </div>
          ) : (
            <Empty className="min-h-[240px] border-0 p-12 text-body">
              <EmptyHeader>
                <EmptyMedia variant="default">
                  <SearchX aria-hidden="true" className="size-6 text-quiet" strokeWidth={1.5} />
                </EmptyMedia>
                <EmptyTitle className="text-[13px] font-normal text-body">
                  {t('viewer.noMatch')}
                </EmptyTitle>
                <EmptyDescription className="text-[12px] text-muted-copy">
                  {t('viewer.tryDifferent')}
                </EmptyDescription>
              </EmptyHeader>
            </Empty>
          )}

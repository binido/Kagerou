  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-[294px] border-hairline bg-raised p-0 text-body">
        <DropdownMenuLabel className="border-b border-hairline px-4 py-3"><span className="type-eyebrow block">Add a source</span><span className="mt-1 block text-[12px] font-normal text-body">Choose how Kagerou receives profiles.</span></DropdownMenuLabel>
        <DropdownMenuItem className="items-start gap-3 rounded-none px-4 py-3" onSelect={() => onChoose('url')}>
          ...
          <span className="min-w-0"><span className="block text-[13px] font-medium text-primary">Subscription URL</span><span className="mt-1 block text-[11px] leading-4 text-muted-copy">Import and refresh profiles from a remote URL.</span></span>
        </DropdownMenuItem>
        ...
        <DropdownMenuItem className="items-start gap-3 rounded-none px-4 py-3" onSelect={() => onChoose('key')}>
          ...
          <span className="min-w-0"><span className="block text-[13px] font-medium text-primary">Single key</span><span className="mt-1 block text-[11px] leading-4 text-muted-copy">Add one key as a single local profile.</span></span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )

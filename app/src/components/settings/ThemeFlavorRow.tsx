...
export const ThemeFlavorRow = forwardRef<HTMLButtonElement, ThemeFlavorRowProps>(
  function ThemeFlavorRow({ theme, active, isFirst, isLast, onSelect, onKeyDown }, ref) {
    ...
    return (
      ...
      >
        <span
          aria-hidden="true"
          className="relative flex h-11 w-3 shrink-0 items-center justify-center"
        >
          <span
            className={cn(
              'absolute left-1/2 top-0 h-1/2 -translate-x-1/2 border-l border-hairline',
              isFirst && 'top-1/2 h-1/2',
            )}
          />
          <span
            className={cn(
              'absolute bottom-0 left-1/2 h-1/2 -translate-x-1/2 border-l border-hairline',
              isLast && 'h-1/2',
            )}
          />
        ...
        <span
          className={cn(
            'flex w-[92px] shrink-0 items-center justify-end gap-1.5 text-[11px]',
            active ? 'font-medium text-lavender-hi' : 'text-muted-copy',
          )}
        >

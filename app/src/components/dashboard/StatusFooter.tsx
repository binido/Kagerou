...
export function StatusFooter({ presets, rules, sources, className }: StatusFooterProps) {
  ...
  const items = [
    enabled.length > 0
      ? t('footer.presets', { names: enabled.map((preset) => preset.label).join(', ') })
      : t('footer.noPresets'),
    t('footer.rules', { count: rules.length }),
    stale > 0
      ? t('footer.sourcesStale', { count: stale, total: sources.length })
      : t('footer.sources', { count: sources.length }),
  ]

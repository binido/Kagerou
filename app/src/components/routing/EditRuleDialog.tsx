    ...
    <Dialog onOpenChange={onOpenChange} open={Boolean(rule)}>
      <DialogContent className="border-[#3b3a45] bg-raised text-primary sm:max-w-[420px]">
        ...
        <form className="space-y-5" onSubmit={submit}>
          <Field><FieldLabel className="text-[12px] text-primary" htmlFor="rule-match">{t('dialog.match')}</FieldLabel><Input className="h-10 border-white/10 bg-surface text-[13px]" id="rule-match" onChange={(event) => setMatch(event.target.value)} placeholder={t('dialog.matchPlaceholder')} value={match} /></Field>

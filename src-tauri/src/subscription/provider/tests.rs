...
fn reads_usage_limit_and_expiry() {
    let info = provider_info(&headers(&[(
...
fn a_number_past_what_sqlite_holds_is_capped() {
    ...
    assert_eq!(info.expires_at, Some(i64::MAX));
}

#[test]
fn no_headers_means_nothing_known() {
    assert_eq!(provider_info(&HeaderMap::new()), ProviderInfo::default());
}

#[test]
fn the_announcement_is_decoded_trimmed_and_cut() {
    // "Привет" in base64.
    let info = provider_info(&headers(&[("announce", "base64:0J/RgNC40LLQtdGC")]));
    assert_eq!(info.announce.as_deref(), Some("Привет"));

    let long = "a".repeat(2000);
    let info = provider_info(&headers(&[("announce", &long)]));
    assert_eq!(info.announce.map(|a| a.len()), Some(ANNOUNCE_MAX_CHARS));

    let info = provider_info(&headers(&[("announce", "base64:%%%")]));
    assert_eq!(info.announce, None);
    let info = provider_info(&headers(&[("announce", "   ")]));
    assert_eq!(info.announce, None);
}

#[test]
fn only_a_web_link_is_kept_as_the_support_url() {
    let info = provider_info(&headers(&[("support-url", " https://t.me/provider ")]));
    ...
    for rejected in [
        ...
        let info = provider_info(&headers(&[("support-url", rejected)]));

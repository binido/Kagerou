...
/// Builds the resolvers and decides which queries reach which one.
///
...
fn dns_block(rules: &[RoutingRule]) -> Value {
    ...
    for rule in rules {
        ...
        // A domain the rules send Direct resolves locally. Resolving it
        // remotely hands back a CDN address near the proxy rather than near
        // the user, sending traffic meant to skip the VPN across the world
        // and back. An ip_cidr rule has no name to resolve at all.
    ...
    json!({
        ...
        "rules": dns_rules,
        "final": "remote",
        // Matches the tunnel that is actually built. The TUN inbound is given
        // a v4 address and nothing else, so an AAAA answer routes a
        // connection into a tunnel that cannot carry it and it hangs.
        "strategy": "ipv4_only",
    })
}

/// Returns the routing rules, preceded by the sniff rule they depend on.
///
/// Without sniffing a connection reaches the rules as an address and a port,
/// so a `domain` or `domain_suffix` rule matches nothing a browser sends -
/// the name is inside the request and nobody has looked. The rule sits there
/// looking configured and routes nothing.
///
...
/// routing rules.
///
/// Each profile's raw URI (`profile.key`) is re-parsed here rather than
/// trusted from cached protocol metadata, so a corrupted key is caught with
/// an error naming the offending profile instead of producing a config the
/// core rejects opaquely at startup.
pub fn generate(input: &ConfigInput) -> Result<Value, ConfigError> {
    ...
    let mut inbounds = vec![json!({
        ...
        // sing-box points the OS proxy here on start and undoes it on a
        // clean stop. Never with TUN. Storage keeps the two exclusive, but a
        // row saved before it did can still hold both.
        "set_system_proxy": input.system_proxy && !input.tun,
    })];
    if input.tun {
        // ponytail: no `interface_name`, sing-box picks a valid one per
        // platform (macOS only accepts `utunN`). Name it explicitly only if
        // users ever need to pin the device.
        inbounds.push(json!({
    ...
    Ok(json!({
        ...
        "route": {
            ...
            // tunnel, which dials the server again. The loop opens thousands
            // of connections and moves zero bytes. Binding outbounds to the
            // default NIC breaks it.
            "auto_detect_interface": true,
            // Mandatory since 1.14 once anything resolves a name, and the
            // proxy servers' own hostnames do. Local of necessity - their
